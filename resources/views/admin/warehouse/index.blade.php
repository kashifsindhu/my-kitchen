...
@section('content')
    ...
    <!-- Content Header State (Page header) -->
    <!-- <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item">
                            <button class="btn btn-primary pull-right" data-bs-toggle="modal" data-bs-target="#addState" class="btn btn-primary pull-right"><i class="fas fa-fw fa-plus"></i>Add State</button>
                        </li>
                    </ol>
                </div>
            </div> -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                State's List (Total States's : <span id="stateTotal">0</span>)
                            </h3>
                        </div> -->
                        <!-- /.card-header -->
                        <!-- <div class="card-body">
    ...
    @include('admin.warehouse.modals._stateModal')
@endsection
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#states').DataTable({
                ...
                drawCallback: function(response) {
                    ...
                    $('#stateTotal').empty();
                    $('#stateTotal').append(response['json'].recordsTotal);
                }
            });
        });
        ...
        $('body').on('click', '.state_edit', function() {
            let id = $(this).attr('data-id');
            $.ajax({
                method: "GET",
                url: "{{ route('state.getdata') }}",
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    'id': id
                },
                success: function(response) {
                    $('body').find('#updateState .form').html(response.html);
