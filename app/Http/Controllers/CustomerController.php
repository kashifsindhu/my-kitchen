...
class CustomerController extends Controller
{
    ...
    public function viewCustomer($id)
    {
        ...
        $customerID = $id;
        $cutt_of_time = Setting::whereName('Cutt_Off_Time')->first();
        if (isset($cutt_of_time)) {
            $cuttOfTime = $cutt_of_time->value;
        } else {
            $cuttOfTime = "2:00:pm";
        }
        if (date('h:i') >  $cuttOfTime) {
            $today = ++$today1;
        } else {
            $today = $today1;
        }
    ...
    public function generatePDF($id)
    {
        ...
        return view('admin.customer.pdfReport', compact('customer', 'products', 'weekDays', 'orders', 'customerID'));
    }
